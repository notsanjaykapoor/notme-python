...
class VariantPrule(sqlmodel.SQLModel, table=True):  # type: ignore
    ...
    effect_amount: pydantic.condecimal(
        max_digits=20, decimal_places=10
    ) = sqlmodel.Field(nullable=False)
    effect_operator: str = sqlmodel.Field(
    ...
    trigger_amount: pydantic.condecimal(
        max_digits=20, decimal_places=10
    ) = sqlmodel.Field(nullable=False)
    trigger_operator: str = sqlmodel.Field(
        index=False, nullable=False
    )  # e.g. 'ge', 'gt', 'le', lt'
    trigger_unit: str = sqlmodel.Field(
        index=False, nullable=False
    )  # e.g. 'amount', 'quantity'
